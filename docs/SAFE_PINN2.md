...
当智能体数量增加时，barrier 交互对数呈二次增长 (N*(N-1)/2)，导致梯度累积问题。Safe PINN PPO 通过以下机制自动处理：

| 参数 | 功能 | 效果 |
|------|------|------|
| `neighbor_normalized_barrier` | 按邻居数量归一化 barrier | 将 barrier 求和改为求平均 |
| `per_agent_grad_clip` | 每个智能体独立裁剪梯度 | 防止单个智能体主导更新 |
| `auto_scale_by_agents` | 自动缩放参数 | 当 n_agents > 4 时，按 4/n 缩放 barrier 权重 |

**示例**: 8 智能体场景
- 原始 barrier 对数: 28 (vs 4智能体的6对)
- 自动缩放因子: 4/8 = 0.5
- 有效 barrier_weight_max: 0.1 × 0.5 = 0.05
- 有效 f_max: 1.0 × 0.5 = 0.5
...
| **梯度稳定性** | 高 | 低 (易爆炸) | 高 (多重优化) |
| **力饱和 (f_max)** | N/A | High (10.0) | Low (1.0, 自动缩放) |
| **多智能体支持** | 是 | 有限 (≤4) | 完整 (自动扩展) |
...
```bash
# 运行 MAPPO (自动使用 Safe PINN PPO，支持多智能体)
